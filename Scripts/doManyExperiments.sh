...
for i in "${lastfutvalarray[@]}"; do
    for m in {0..14}; do
        ...
        echo "node RequestGenerator.js TestingRegressionDump.csv $i $i $m Elastic AllRSU 1"
        node RequestGenerator.js TestingRegressionDump.csv "$i" "$i" "$m" "Elastic" AllRSU 1
